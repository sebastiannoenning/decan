...
#       pyside6-uic --from-imports resources/ui/dialogs/check_password_dialog.ui -o client/views/dialogs/check_password_dialog_ui.py, or
#       pyside6-uic --from-imports dialogs/check_password_dialog.ui -o ../client/views/dialogs/check_password_dialog_ui.py.py
#
# For icons:
...
from client.views.dialogs.check_password_dialog_ui import Ui_check_password_dialog
...
class CheckPassword(QDialog):
    ...
    def _validatePassword(self):
        ...
        valid = self.model.validatePasswordsRoute(self.username, password)
        if valid: self.accept()
        else:
            if not self._Ui.diagnostic_label.isVisibleTo(self): self._Ui.diagnostic_label.setVisible(True)
            self.setDiagnotisticLabelEnabled()
